...
object Part1Basics {
    ...
    fun secondLargest(list: List<Int>): Int {
        ...
        val secondLargest = sortedList[1]
    ...
    fun filterNegativeNumbers(numbers: List<Int>): List<Int> {
        val getFilteredList = numbers.filter {it >= 0}
    ...
    fun charFrequency(str: String): Map<Char, Int> {
        val mappedStr = str.filter{it.isLetter()}
            val freguency = mappedStr.groupBy{it}
